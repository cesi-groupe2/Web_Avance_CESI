...
func HandlerMicroServRestaurantRoads(server *gin.Engine, sqlClient *gorm.DB) *gin.RouterGroup {
	...
	restaurant.DELETE("/:restaurantId", func(ctx *gin.Context) {
		restaurantService.DeleteRestaurant(ctx, sqlClient)
	})
	...
	restaurant.GET("/:restaurantId/proprio", func(ctx *gin.Context) {
		restaurantService.GetProprioByRestaurantId(ctx, sqlClient)
	})
